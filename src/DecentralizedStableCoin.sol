...
contract DecentralizedStableCoin is ERC20Burnable, Ownable {
    ...
    //makes _initialOwner the owner of this token

    //we override burn function so that onlu Owner can burn tokens and also we can check if the amount to be
    // burned is more than zero
    ...
    function mint(address _to, uint256 _amount) external onlyOwner returns (bool) {
        ...
        } //no need to check for this since it has already been checked in _mint function
        if (_amount <= 0) {
            revert DecentralizedStableCoin__AmountMustBeMoreThanZero();
        }
        _mint(_to, _amount);
